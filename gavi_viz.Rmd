...
updt_plot_acres_allclass <- ggplot() +
 geom_line(data = subset(rev_plot_predict, variable == "acres"), aes(x = year, y = value, group = land_class, color = land_class), size = .7, linetype = "dashed") +
  geom_point(data = tstock_plot_acres, aes(x = year, y = value, group = land_class, color = land_class), size = 3.3) +
  theme_minimal() +
  scale_color_manual(values = c("lightsteelblue", "goldenrod", "lightslategrey", "darkred", "sandybrown", "darkolivegreen3", "green4", "cornflowerblue", "purple4")) +
  labs(color = "Land Class",
       y = "Acres",
...
updt_plot_acres_ag <- ggplot() +
  geom_line(data = subset(rev_plot_predict, !land_class %in% c("Rangeland", "Total (all classes)") &  variable == "acres"), aes(x = year, y = value, group = land_class, color = land_class), size = 1.2, linetype = "dashed") +
  geom_point(data = subset(tstock_plot_acres, !land_class %in% c("Rangeland", "Total (all classes)")), aes(x = year, y = value, group = land_class, color = land_class), size = 3.3) +
  theme_minimal() +
  scale_color_manual(values = af_colors) +
  labs(color = "Land Class",
       y = "Acres",
