...
reclass_levels <- data.frame(levels(all_coord_vals$reclass_fact), c(1:14))

all_coord_spatial <- all_coord_vals %>% 
  as("Spatial")

# crs(all_coord_vals) <- ("+proj=longlat +lat_1=34.03333333333333 +lat_2=35.46666666666667 +lat_0=33.5 +lon_0=-118 +x_0=2000000 +y_0=500000.0000000002 +ellps=GRS80 +datum=NAD83 +to_meter=0.3048006096012192 +no_defs")

rast_landclass <- raster::rasterize(all_coord_vals, sb_mask, field = 'total_stock', fun = mean)
