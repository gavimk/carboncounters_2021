...
# Total Acres
for (name in names) {
lm_total_acre_loop <- lm([[name]] ~ year, data = all_ag_df)
lm_total_acre_tidy_loop <- broom::tidy(lm_total_acre_loop)

# Now we will use these models to predict acreage (total and per crop type) in 2030

# Next, we'll predict values for these years using our model
# Total acres
predict_total_run_loop <- predict(lm_total_acre, newdata = predict_df_30_acres)
predict_total_df_loop <- data.frame(predict_df_30_acres, predict_total_run_loop)
predict_total_df_loop <- predict_total_df_loop
